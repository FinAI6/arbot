...
class ArBotGUI:
    def __init__(self, config: Config, database: Database, exchanges: Dict = None):
        self.config = config
        self.database = database
        self.exchanges = exchanges or {}  # Use pre-initialized exchanges if provided
        self.strategy = None
        self.trader = None
        self.simulator = None
    ...
    def display_arbitrage_data(self):
        ...
        for row in sorted_data:
            ...
            # Check if this is an anomalous spread (use config threshold)
            max_spread_threshold_pct = self.config.arbitrage.max_spread_threshold * 100
            is_anomalous = spread_pct >= max_spread_threshold_pct
            ...
            higher_exchange = row.get('higher_exchange', 'N/A')
            higher_price = row.get('higher_price', row.get('price', 0))  # Fallback to 'price' if 'higher_price' missing
            price_diff = row.get('price_diff', 0)
            
            # Format price with difference
            price_display = f"{higher_price:.6f} (+{price_diff:.6f})"
            
            # Get moving average for the higher exchange (with safe access)
            exchange1 = row.get('exchange1', '')
            higher_exchange_name = row.get('higher_exchange', '')
            ma_value = row.get('ma1') if higher_exchange_name == exchange1.upper() else row.get('ma2')
            ma_display = f"{ma_value:.6f}" if ma_value else "N/A"
            
            # Get trend for the higher exchange (with safe access)
            trend = row.get('trend1', '→') if higher_exchange_name == exchange1.upper() else row.get('trend2', '→')
    ...
    async def _initialize_components_async(self):
        ...
        try:
            ...
            # Initialize exchanges if not already provided
            if not self.exchanges:
                logger.info("Initializing exchanges from config...")
                ...
                logger.info(f"Using pre-initialized exchanges: {list(self.exchanges.keys())}")
            
            # Register ticker callbacks for all exchanges (whether new or pre-initialized)
            for exchange_name, exchange in self.exchanges.items():
                print(f"📝 {exchange_name} 콜백 등록 중...")
                logger.info(f"About to register callbacks for {exchange_name} exchange (type: {type(exchange).__name__})")
                
                # 1. For spreads view (always needed)
                def create_spreads_callback(exchange_name):
                    async def callback(ticker):
                        await self._on_ticker_for_spreads(ticker, exchange_name)
                    return callback
                
                spreads_callback = create_spreads_callback(exchange_name)
                exchange.on_ticker(spreads_callback)
                print(f"✅ {exchange_name} spreads 콜백 등록 완료")
                logger.info(f"Registered spreads callback for {exchange_name}: {spreads_callback}")
                
                # 2. For data storage (if enabled)
                if self.config.database.store_ticker_data:
                    def create_storage_callback(exchange_name):
                        async def callback(ticker):
                            await self._on_ticker_for_storage(ticker, exchange_name)
                        return callback
                    
                    storage_callback = create_storage_callback(exchange_name)
                    exchange.on_ticker(storage_callback)
                    print(f"✅ {exchange_name} storage 콜백 등록 완료")
                    logger.info(f"Registered storage callback for {exchange_name}: {storage_callback}")
                
                # Check if callbacks are properly registered
                callback_count = len(exchange._callbacks.get('ticker', []))
                print(f"📊 {exchange_name} 총 콜백 수: {callback_count}개")
                logger.info(f"Exchange {exchange_name} now has {callback_count} ticker callbacks registered")
                
                # Verify callback registration by checking internal state
                if hasattr(exchange, '_callbacks') and 'ticker' in exchange._callbacks:
                    logger.info(f"✅ {exchange_name} callback verification passed: {len(exchange._callbacks['ticker'])} callbacks in _callbacks['ticker']")
                else:
                    logger.warning(f"⚠️ {exchange_name} callback verification failed: no _callbacks['ticker'] found")
            ...
            logger.info("Getting dynamic symbols from exchanges (ignoring config static symbols)")
            ...
            if config_symbols:
                logger.info(f"Config had {len(config_symbols)} static symbols, now using {len(self.dynamic_symbols)} dynamic symbols")
            
            logger.info(f"Monitoring {len(self.dynamic_symbols)} symbols: {self.dynamic_symbols[:10]}...")
            
            # Auto-connect WebSocket for spreads view (no need to wait for "Start Trading")
            if self.exchanges and self.dynamic_symbols:
                ...
                if symbols_to_monitor:
                    print(f"📋 심볼 예시: {', '.join(symbols_to_monitor[:10])}{'...' if len(symbols_to_monitor) > 10 else ''}")
                
                for exchange_name, exchange in self.exchanges.items():
                    try:
                        logger.info(f"Connecting {exchange_name} WebSocket to {len(symbols_to_monitor)} symbols...")
                        logger.info(f"First 10 symbols for {exchange_name}: {symbols_to_monitor[:10]}")
                        
                        await exchange.connect_ws(symbols_to_monitor)
                        
                        # Verify connection status
                        if exchange.connected:
                            print(f"✅ {exchange_name.upper()} WebSocket 연결 완료 (connected={exchange.connected})")
                            logger.info(f"WebSocket connected for {exchange_name} with {len(symbols_to_monitor)} symbols")
                            
                            # Verify internal WebSocket state
                            if hasattr(exchange, 'symbols'):
                                logger.info(f"{exchange_name} WebSocket internal symbols count: {len(exchange.symbols)}")
                            if hasattr(exchange, 'ws_connection'):
                                logger.info(f"{exchange_name} WebSocket connection object: {exchange.ws_connection is not None}")
                        else:
                            print(f"⚠️ {exchange_name.upper()} WebSocket 연결 시도했으나 connected=False")
                            logger.warning(f"WebSocket connection attempted for {exchange_name} but connected flag is False")
                    except Exception as e:
                        print(f"❌ {exchange_name} WebSocket 연결 실패: {e}")
    ...
    async def _start_trading(self):
        ...
        try:
            ...
            # Skip WebSocket re-connection if already connected for spreads view
            ...
            for exchange_name, exchange in self.exchanges.items():
                if not exchange.connected:
                    try:
                        logger.info(f"Connecting {exchange_name} WebSocket for trading...")
                        await exchange.connect_ws(symbols_to_monitor)
                        print(f"✅ {exchange_name.upper()} WebSocket 거래 연결 완료")
                    except Exception as e:
                        print(f"❌ {exchange_name} WebSocket 거래 연결 실패: {e}")
                        logger.error(f"Failed to connect WebSocket for trading on {exchange_name}: {e}")
                else:
                    logger.info(f"{exchange_name} WebSocket already connected, skipping re-connection")
                    print(f"ℹ️ {exchange_name.upper()} WebSocket 이미 연결됨")
    ...
    def create_trading_settings(self, parent):
        ...
        try:
            ...
            spread_threshold = getattr(self.config.arbitrage, 'max_spread_threshold', 1.0) * 100
    ...
    async def _reinitialize_exchanges(self):
        ...
        try:
            ...
            for exchange_name, exchange_config in self.config.exchanges.items():
                ...
                exchange.on_ticker(create_spreads_callback(exchange_name))
                ...
                if self.config.database.store_ticker_data:
                    ...
                    exchange.on_ticker(create_storage_callback(exchange_name))
    ...
    async def _on_ticker_for_spreads(self, ticker, exchange_name):
        """Update current_prices with WebSocket ticker data for spreads view"""
        try:
            # Log first few ticker updates for debugging
            if not hasattr(self, '_ticker_debug_count'):
                self._ticker_debug_count = {}
            if exchange_name not in self._ticker_debug_count:
                self._ticker_debug_count[exchange_name] = 0
            ...
            # Update price data
            self.current_prices[exchange_name][ticker.symbol] = {
                'bid': ticker.bid,
                'ask': ticker.ask,
                'timestamp': ticker.timestamp
            }
            ...
            if total_symbols <= 20:  # Log for first 20 total symbols
                logger.info(f"Updated spreads data: {exchange_name} {ticker.symbol} {ticker.bid:.6f}/{ticker.ask:.6f}")
            elif total_symbols == 21:  # Log once when we hit the limit
                unique_symbols = set()
                for symbols in self.current_prices.values():
                    unique_symbols.update(symbols.keys())
                logger.info(f"✅ Spreads view now receiving data for {len(unique_symbols)} unique symbols across {len(self.current_prices)} exchanges")
            
        except Exception as e:
            logger.error(f"Error updating spreads data: {e}")

    async def _periodic_batch_flush(self):
        """Periodically flush ticker batch if enough time has passed"""
        try:
            if not self.ticker_buffer:
                return
            ...
            if time_since_batch >= self.config.database.ticker_batch_interval_seconds:
                await self._flush_ticker_batch()
    ...
    async def _update_data(self):
        ...
        try:
            ...
            # Debug: Log _update_data execution
            if not hasattr(self, '_update_data_count'):
            ...
            # Initialize exchange dictionaries for WebSocket data
            if self.exchanges:
                for exchange_name in self.exchanges.keys():
                    if exchange_name not in self.current_prices:
                        self.current_prices[exchange_name] = {}
                        
            # Note: Ticker data is now populated via WebSocket callbacks only
            # This eliminates REST API rate limit issues
    ...
    def update_price_display(self):
        ...
        # Debug logging to understand why spreads aren't updating
        if not hasattr(self, '_price_display_debug_count'):
        ...
        if len(self.current_prices) >= 1 and total_symbols > 0:
            ...
            # If we only have one exchange, show bid-ask spreads for that exchange
            if len(exchange_names) == 1 and not arbitrage_rows:
                exchange_name = exchange_names[0]
                logger.info(f"Using single exchange mode for {exchange_name} with {len(exchange_data)} symbols")
                exchange_data = self.current_prices[exchange_name]
                
                for symbol, price_data in exchange_data.items():
                    # Check data staleness
                    current_time = time.time()
                    if current_time - price_data.get('timestamp', 0) > 60:
                        continue
                    ...
                    if bid > 0 and ask > 0:
                        ...
                        self.moving_average_manager.update_price(f"{symbol}_{exchange_name}", mid_price)
                        
                        # Get moving average and trend
                        ma = self.moving_average_manager.get_moving_average(f"{symbol}_{exchange_name}")
                        trend = self.moving_average_manager.get_price_trend(f"{symbol}_{exchange_name}")
                        
                        arbitrage_rows.append({
                            'symbol': symbol,
                            'higher_exchange': exchange_name.upper(),
                            'price': mid_price,
                            'higher_price': mid_price,  # Add missing key for display compatibility
                            'price_diff': spread_abs,
                            'spread_pct': spread_pct,
                            'actual_arbitrage_pct': spread_pct,  # For single exchange, spread = arbitrage opportunity
                            'ma1': ma,
                            'ma2': ma,  # Same for both since it's one exchange
                            'trend1': trend,
                            'trend2': trend,
            ...
            # Debug logging for arbitrage data creation
            if self._price_display_debug_count % 10 == 1:
                logger.info(f"🔍 Arbitrage calculation result: {len(arbitrage_rows)} rows generated")
                if not arbitrage_rows:
                    # Log why no arbitrage data was generated
                    exchange_names = list(self.current_prices.keys())
            ...
            if arbitrage_rows:
                ...
                for exchange_name in exchange_names:
                    symbols = self.current_prices[exchange_name]
    ...
    def update_opportunities_display(self):
        ...
        sorted_opportunities.sort(key=lambda x: x.profit_percent, reverse=True)
        
        # Add top opportunities (limit to 20 for display)
        for i, opp in enumerate(sorted_opportunities[:20]):
            profit_pct = opp.profit_percent * 100
    ...
    async def get_common_symbols_with_volume(self) -> List[str]:
        ...
        for exchange_name, exchange in self.exchanges.items():
            try:
                ...
                # Try multiple methods to get symbols, starting with the most reliable
                symbols_found = False
                
                # Method 1: Try get_symbols first (if not Binance or if Binance API is working)
                if hasattr(exchange, 'get_symbols') and (exchange_name != 'binance' or 
                    not self._is_binance_rate_limited()):
                    try:
                        ...
                        if symbols:
                            ...
                            logger.info(f"{exchange_name}: Found {len(usdt_symbols)} USDT pairs via get_symbols")
                            symbols_found = True
                    except Exception as e:
                        logger.warning(f"{exchange_name}: get_symbols failed: {e}")
                
                # Method 2: Try get_all_tickers if get_symbols failed
                if not symbols_found and hasattr(exchange, 'get_all_tickers'):
                    try:
                        tickers = await asyncio.wait_for(exchange.get_all_tickers(), timeout=10.0)
                        if tickers:
                            ...
                            if symbols:
                                ...
                                logger.info(f"{exchange_name}: Found {len(symbols)} USDT pairs from tickers")
                                symbols_found = True
                    except Exception as e:
                        logger.warning(f"{exchange_name}: get_all_tickers failed: {e}")
                
                # Method 3: Use predefined symbol list for known exchanges
                if not symbols_found:
                    logger.warning(f"{exchange_name}: Using fallback symbol list")
                    ...
                    logger.info(f"{exchange_name}: Using {len(fallback_symbols)} fallback USDT pairs")
                    
            except asyncio.TimeoutError:
                logger.warning(f"Timeout getting symbols from {exchange_name}, using fallback")
                fallback_symbols = self._get_fallback_symbols_for_exchange(exchange_name)
                exchange_symbols[exchange_name] = set(fallback_symbols)
                continue
            except Exception as e:
                logger.error(f"Error getting symbols from {exchange_name}: {e}, using fallback")
                fallback_symbols = self._get_fallback_symbols_for_exchange(exchange_name)
                exchange_symbols[exchange_name] = set(fallback_symbols)
                continue
        
        # Process results based on number of successful exchanges
        if not exchange_symbols:
            logger.error("Failed to get symbols from any exchange, using fallback")
            return ["BTCUSDT", "ETHUSDT", "XRPUSDT", "ADAUSDT", "SOLUSDT"]
        
        if len(exchange_symbols) == 1:
            # Single exchange: use all its symbols
            exchange_name = list(exchange_symbols.keys())[0]
            all_symbols = list(exchange_symbols[exchange_name])
            logger.info(f"Single exchange mode: Using all {len(all_symbols)} symbols from {exchange_name}")
            
            # Sort by symbol name for consistency and take up to max_symbols
            all_symbols.sort()
            max_symbols = getattr(self.config.arbitrage, 'max_symbols', 200)
            selected_symbols = all_symbols[:max_symbols]
            
        else:
            # Multiple exchanges: find common symbols
            logger.info("Multi-exchange mode: Finding common symbols...")
            ...
            if not common_symbols:
                logger.warning("No common symbols found, using largest exchange")
                # Use symbols from exchange with most symbols
                largest_exchange = max(exchange_symbols.keys(), 
                                     key=lambda x: len(exchange_symbols[x]))
                common_symbols = exchange_symbols[largest_exchange]
                logger.info(f"Using {len(common_symbols)} symbols from largest exchange: {largest_exchange}")
            else:
                logger.info(f"Found {len(common_symbols)} common symbols")
            
            # Convert to sorted list and limit
            selected_symbols = sorted(list(common_symbols))
            max_symbols = getattr(self.config.arbitrage, 'max_symbols', 200)
            selected_symbols = selected_symbols[:max_symbols]
        
        logger.info(f"Final selection: {len(selected_symbols)} symbols")
        if selected_symbols:
            logger.info(f"First 20 symbols: {selected_symbols[:20]}")
            logger.info(f"Last 10 symbols: {selected_symbols[-10:]}")
        
        return selected_symbols

    def _is_binance_rate_limited(self) -> bool:
        """Check if Binance is currently rate limited"""
        # Simple heuristic - you could store rate limit state or check recent errors
        return False  # For now, always try Binance
    
    def _get_fallback_symbols_for_exchange(self, exchange_name: str) -> List[str]:
        """Get fallback symbol list for specific exchange"""
        # Common major symbols that should exist on most exchanges
        major_symbols = [
            "BTCUSDT", "ETHUSDT", "BNBUSDT", "XRPUSDT", "ADAUSDT", "SOLUSDT", 
            "DOGEUSDT", "MATICUSDT", "AVAXUSDT", "DOTUSDT", "LTCUSDT", "LINKUSDT", 
            "UNIUSDT", "BCHUSDT", "XLMUSDT", "ATOMUSDT", "FILUSDT", "ETCUSDT", 
            "TRXUSDT", "VETUSDT", "AAVEUSDT", "MKRUSDT", "COMPUSDT", "SUSHIUSDT", 
            "CRVUSDT", "YFIUSDT", "SNXUSDT", "1INCHUSDT", "ENJUSDT", "MANAUSDT", 
            "SANDUSDT", "CHZUSDT", "GALAUSDT", "LRCUSDT", "BATUSDT", "ZRXUSDT", 
            "ANKRUSDT", "ALGOUSDT", "ZILUSDT", "ONEUSDT", "ICPUSDT", "FTMUSDT", 
            "THETAUSDT", "HBARUSDT", "EOSUSDT", "ZECUSDT", "WAVESUSDT", "ZENUSDT", 
            "RVNUSDT", "NEOUSDT", "DASHUSDT", "XMRUSDT", "XTZUSDT", "QTUMUSDT", 
            "KSMUSDT", "DGBUSDT", "SCUSDT", "NKNUSDT", "ARDRUSDT", "CELOUSDT"
        ]
        
        # Exchange-specific adjustments
        if exchange_name == 'binance':
            # Binance typically has the most symbols
            return major_symbols
        elif exchange_name == 'bybit':
            # Bybit may not have all symbols, filter out known problematic ones
            problematic = {
                "MLNUSDT", "DEXEUSDT", "BANDUSDT", "CTKUSDT", "STORJUSDT", 
                "KNCUSDT", "RSRUSDT", "RLCUSDT", "REQUSDT", "OGNUSDT"
            }
            return [s for s in major_symbols if s not in problematic]
        else:
            # For other exchanges, use conservative list
            return major_symbols[:30]  # First 30 most common symbols
    ...
    async def _cleanup_and_quit(self):
        ...
        try:
            ...
            # Flush any remaining ticker batch data
            if (self.config.database.store_ticker_data and \
                self.config.database.ticker_storage_mode == "batch" and \
                self.ticker_buffer):
                logger.info(f"Flushing {len(self.ticker_buffer)} remaining ticker records")
                await self._flush_ticker_batch()
            # Stop trading if active
            if self.trading_active:
                await self._stop_trading()
...
async def run_gui(config: Config, database: Database, exchanges: Dict = None):
    """Run the GUI application with pre-initialized exchanges"""
    gui = None
    try:
        ...
        def start_gui():
            nonlocal gui
            gui = ArBotGUI(config, database, exchanges)
            gui.run()
